...
reportname="monthlyworkflowfailures_$currentday.txt"
...
echo "$clusterlist" | while IFS="}" read -ra cluster; do
	reporthealthout=$(ssh -n -o BatchMode=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i $userkey $username@${cluster[1]} reportfailedjobs -d31 -j reprovision -c | awk -F"," '{ print $1","$4","$6","$7 }' | column -t -s,  2>/dev/null)
	if [ $? -ne 0 ]; then
...
echo 'Subject: Actifio Monthly Database Refresh failure Summary
MIME-Version: 1.0
...
find /root -name "monthlyworkflowfailures_*.txt" -mtime +8 -exec rm -f {} \;
