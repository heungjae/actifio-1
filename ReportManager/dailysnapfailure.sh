...
reportname="dailysnapfailure_$currentday.txt"
...
echo "$clusterlist" | while IFS="}" read -ra cluster; do
        reporthealthout=$(ssh -n -o BatchMode=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i $userkey $username@${cluster[1]} reportfailedjobs -j snapshot -d1  2>/dev/null)
	if [ $? -ne 0 ]; then
...
echo 'Subject: Daily Snap Failures
MIME-Version: 1.0
...
find /root -name "dailysnapfailure_*.txt" -mtime +8 -exec rm -f {} \;
