...
reportname="monthlypoolcapacityandmdlusage_$currentday.txt"
...
echo "$clusterlist" | while IFS="}" read -ra cluster; do
        reporthealthout=$(ssh -n -o BatchMode=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i $userkey $username@${cluster[1]} reportapps -c -g  | awk -F";" '{ print $1","$3","$9","$13","$15","$17 }' | column -t -s, 2>/dev/null)
	if [ $? -ne 0 ]; then
...
echo 'Subject: Actifio Pool & MDL Usage
MIME-Version: 1.0
...
find /root -name "monthlypoolcapacityandmdlusage_*.txt" -mtime +8 -exec rm -f {} \;
