...
recipient="aaron.tully@actifio.com,michael.chew@actifio.com"
...
reportname="monthlyworkflows_$currentday.txt"
...
echo "$clusterlist" | while IFS="}" read -ra cluster; do
	reporthealthout=$(ssh -n -o BatchMode=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i $userkey $username@${cluster[1]} reportmounts -d31 -r -c | awk -F"," '{ print $1","$4","$5","$6","$8 }' | column -t -s,  2>/dev/null)
	if [ $? -ne 0 ]; then
...
echo 'Subject: Actifio Monthly Database Refresh Summary
MIME-Version: 1.0
...
find /root -name "monthlyworkflows_*.txt" -mtime +8 -exec rm -f {} \;
