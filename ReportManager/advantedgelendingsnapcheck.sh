...
recipient="nab_alerts@actifio.com"
#blindrecipient=
#recipient=
################################################################################################
...
reportname="advantedgwlendingdailycapture_$currentday.txt"
...
echo "$clusterlist" | while IFS="}" read -ra cluster; do
        reporthealthout=$(ssh -n -o BatchMode=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i $userkey $username@${cluster[1]} reportsnaps -d1 -l 25 | grep -w -i -e 'ODS' -e 'CR' -e 'cwfDataWarehouse' -e 'TMADB' -e 'ILMSDB' -e 'StarData' -e 'MortgageReporting' -e 'mortgageinfrastructure' -e 'Valuationdata' -e 'DecisionData' -e 'SVM' -e 'ArchiveDB' -e 'ArchiveTMADB' -e 'Discharge' -e 'FusionFramework' -e 'JetStream' -e 'ProductConfiguration' -e 'FDIMessaging' -e 'WorkDB' -e 'CreditCheck' -e 'IWFLIXIData' -e 'LMIData' -e 'StarRules' -e 'CWFMailData' -e 'TBSData' | wc -l  2>/dev/null)
	if [ $? -ne 0 ]; then
...
echo 'Subject: Avantedge lending daily snap count
MIME-Version: 1.0
...
find /root -name "advantedgwlendingdailycapture_*.txt" -mtime +8 -exec rm -f {} \;
