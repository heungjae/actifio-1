...
reportname="monthlymaskingworkflow_$currentday.txt"
...
echo "$clusterlist" | while IFS="}" read -ra cluster; do
        reporthealthout=$(ssh -n -o BatchMode=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i $userkey $username@${cluster[1]} reportworkflows -s -d31 -c | { head -1; egrep -i 'AUTH3SQ024NABWI|AUTH3SQ026NABWI|AUTH3SQ027NABWI|auth3sq029nabw|aute3od153naboi'; } | awk -F"," '{ print $1","$3","$5","$7","$9 }' | column -t -s, 2>/dev/null)
	if [ $? -ne 0 ]; then
...
echo 'Subject: Actifio Masking Workflow Monthly Summary
MIME-Version: 1.0
...
find /root -name "monthlymaskingworkflow_*.txt" -mtime +8 -exec rm -f {} \;
